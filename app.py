...
def load_forecasts():
    ...
    # Ensure both FOB_China and FOB_Japan columns exist
    for df in [var, reg, lstm]:
        if "FOB_China" not in df.columns or "FOB_Japan" not in df.columns:
            if "FOB_China" in df.columns:
                df["FOB_Japan"] = df["FOB_China"]
            elif "FOB_Japan" in df.columns:
                df["FOB_China"] = df["FOB_Japan"]
...
st.write("Columns in filtered_df:", model_df.columns.tolist())  # Debug line
...
for country in ["China", "Japan"]:
    ...
    if fob_col in filtered_df.columns:
